...
func_valid_daemon(){
  if [[ $(screen -list|grep nikto|wc -l) -eq '0' ]]; then
    echo
...
func_valid_dsession(){
  if [[ $(screen -list|grep nikto|grep Detached|wc -l) -eq '0' ]]; then
    echo
...
func_run(){
  ...
  # Daemonize Statement
  if [[ ${daemon} -eq '2' ]]; then
    screen -dmS nikto sh -c 'echo [*] Daemonizing... ; exec bash'
    screen -S nikto -p 0 -X stuff "echo ; ${0} ${args} $(printf \\r)"
    screen -r
    exit 0
  fi
...
# Manage Nikto Threads
  ...
  do
    ...
    nikto -host ${host} -ask no -output ${file}.html -F htm -timeout ${timeout} > ${file}.txt &
    ((host_num++))
  done
...
do
  case ${opt} in
    f)
      ...
      ;;
    d)
      daemon=$(ps aux|grep -i nikto|grep -v 'grep '|wc -l)
      ;;
    w)
      threads=${OPTARG}
